...
function rawBody (request, reply, options, parser, done) {
  ...
  const shouldPreAllocateBuffer = parser.asBuffer === true &&
    payload === request.raw &&
  ...
  let body = asString ? '' : shouldPreAllocateBuffer ? Buffer.allocUnsafe(contentLength) : []
  let bodyOffset = 0
  ...
  function onData (chunk) {
    ...
    if (asString) {
      body += chunk
    } else if (shouldPreAllocateBuffer) {
      if (receivedLength > body.length) {
        payload.removeListener('data', onData)
        payload.removeListener('end', onEnd)
        payload.removeListener('error', onEnd)
  ...
  function onEnd (err) {
    ...
    if (!asString && shouldPreAllocateBuffer === false) {
